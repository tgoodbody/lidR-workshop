...
las = readLAS("~/Documents/ALS data/Haliburton dataset/Landscape LiDAR/CN_681_5015.las")
plot(las)
...
ctg = readLAScatalog("~/Documents/ALS data/Haliburton dataset/Landscape LiDAR/")
plot(ctg)
...
ctg <- readLAScatalog("~/Documents/ALS data/Montmorency dataset/las/")
...
shade = function(dtm)
{
  ...
  shadeddtm = mdtm %>% rayshader::sphere_shade(texture = "bw")
...
p = shapefile("~/Documents/lidR demo/lidR_demo_points.shp")
p

plot(ctg)
plot(p, add = T, col = "red", pch = 8)

las = lasclip(ctg, p, radius = 30)
plot(las[[1]])
plot(las[[2]])
...
p = shapefile("~/Documents/lidR demo/lidR_demo_polygon.shp")
p
...
las <- readLAS("~/Documents/ALS data/Fichiers las divers/Example.las")
las <- lasground(las, csf(), last_returns = FALSE)
las <- lasnormalize(las, tin())
plot(las)

ttops <- tree_detection(las, lmf(5, 5))
ttops
...
ttops1 <- tree_detection(chm1, lmf(5,5))
...
ctg <- readLAScatalog("~/Téléchargements/Farm_A/")
ctg
plot(ctg)
...
# ---- Individual tree detection ----
ttops = tree_detection(ctg, lmf(3, 5))
...
plot(las, color = "treeID", colorPalette = height.colors(200))

tree145 = lasfilter(las, treeID == 145)
plot(tree145)
